...
-(id)initWithNumberOfComments:(NSUInteger)comments
{
	...
	_image.urlPath = @"bundle://commentView2.png";
...
-(void)layoutSubviews
{
	...
	TTDINFO("Highlighted %d", self.highlighted);
	
	[self addSubview:_image];
	_image.userInteractionEnabled = NO;
	
	[self addSubview:_numbers];
	_numbers.frame = CGRectMake(24, 4, 75, 17);
...
-(void)setNumberOfComments:(NSUInteger)numComments
{
	if (numComments == 1) {
		...
		_numbers.text = NSLocalizedString(@"comment", nil);
	}
